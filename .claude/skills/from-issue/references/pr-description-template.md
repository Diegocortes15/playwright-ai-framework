...
- **Verification — say what scope ran and why**: Step 10 asks the repository's `scripts/affected-specs.sh` what the change reaches. Name the answer in the Test run list — `- **Scope:** affected specs (`<list>`) — resolved from <what changed>.` or `- **Scope:** full suite — the change reaches shared code, which cannot be scoped.` A reader must never have to infer from the number of lines whether a narrow run was a decision or an oversight.
