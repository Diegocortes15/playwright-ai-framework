...
| `@all-users` | every `chromium-<user>` project (`chromium-standard`, `chromium-problem`, `chromium-error`) | User-agnostic flows. **Available, and used by zero specs today** — every test is pinned to one user. `footer` is the natural first candidate: a copyright line should read the same for anybody                         |
