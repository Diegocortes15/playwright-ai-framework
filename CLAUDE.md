...
| `@all-users` | every `chromium-<user>` project (`chromium-standard`, `chromium-problem`, `chromium-error`) | User-agnostic flows. **In use since SW-20**, by `tests/footer/footer.spec.ts` and nothing else — every other test is pinned to one user. The footer was the natural first candidate: a copyright line should read the same for anybody                         |
